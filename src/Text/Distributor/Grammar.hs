...
class Tokenized a b p | p -> a, p -> b where token :: p a b

data Token a b s t where Token :: Token a b a b
...
restOfStream :: (Tokenized a b p, Stream s t a b, Distributor p) => p s t
